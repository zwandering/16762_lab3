...
class YOLOEObjectDetector(Node):
    ...
    def get_goal_pose(self, detections, target_idx=0):
        ...
        centroid = detection["centroid"]  # (x, y) in pixel coordinates
        
        # Get the depth value at the centroid
        z_depth = self.latest_depth[centroid[1], centroid[0]]  # depth image is indexed [row, col] = [y, x]
        
        # Project the centroid to 3D coordinates
        xyz_3d = detection_utils.pixel_to_3d(centroid, z_depth, self.latest_color_cam_info)
